...
Replace `hermes` with `opencode`, `openclaw`, or `kimi-code` for those clients.

- This section is the legacy project-scope path. These clients keep MCP config in profile/global or client-specific locations, so project `setup mcp` PREVIEWS the exact registration command instead of writing it. Default global onboarding is different: `onboard --agent hermes` writes the Hermes MCP profile plus its observe-only v1 shell hooks (one-time consent and gateway restart still required), `onboard --agent opencode` writes OpenCode MCP config, global rules, and its observe-only v1 plugin, and `onboard --agent kimi-code` writes the user-level `mcpServers` registration in `$KIMI_CODE_HOME/mcp.json` plus the `$KIMI_CODE_HOME/AGENTS.md` directive (`setup mcp --agent kimi-code --write` applies that same registration on its own). None of these clients has a generic Evidence v2 manifest adapter.
...
- Kimi Code reads MCP servers from `~/.kimi-code/mcp.json` (`$KIMI_CODE_HOME/mcp.json`; project level `.kimi-code/mcp.json`) — not `config.toml`, which carries provider credentials and has no MCP section — and its standing directive from `$KIMI_CODE_HOME/AGENTS.md`. `agentacct setup mcp --agent kimi-code --write` merges the `mcpServers.agentacct` entry into that user-level file for you (preserving every other server and key, and collapsing a stale pre-rename `agent-sentinel`/`agent-chronicle` entry), or `agentacct onboard --scope global --agent kimi-code` writes both the registration and the directive. Because that file is user-level, both paths bind the machine-wide store (`agentacct setup global-store-path` prints it) and an absolute command path — not this project's store, and not a bare `agentacct` name a GUI-launched session may not resolve; `setup preview --agent kimi-code --user` shows the same entry. Without `--write`, `agentacct setup mcp --agent kimi-code` previews the exact entry. Then start a new session — running sessions never register a newly added server.
...
- Kimi Code: local usage import from ~/.kimi-code (session ids from its session_index.jsonl index, token counters summed from the per-request usage.record events in each session's agents/*/wire.jsonl streams; tokens are client_reported and cost is only a local pricing-table estimate marked ≈), plus MCP self-reporting — `agentacct setup mcp --agent kimi-code --write` merges the `mcpServers.agentacct` entry into the user-level `$KIMI_CODE_HOME/mcp.json` (default `~/.kimi-code/mcp.json`; the project-level `.kimi-code/mcp.json` is not written) and `agentacct onboard --scope global --agent kimi-code` installs all three legs — that MCP registration, the record-your-work directive into `$KIMI_CODE_HOME/AGENTS.md`, and an observe-only hook bridge (`agentacct hooks kimi-code install`) merged into the user's `~/.kimi-code/config.toml` as Kimi Code `[[hooks]]` entries (only event, matcher, command, and timeout are allowed there) — so a new Kimi Code session has the server, the directive, and the bridge (without `--write`, `agentacct setup mcp --agent kimi-code` previews the exact entry). The hook receives Kimi Code's authoritative session id on stdin and writes a tool-activity tick plus a hook-context file on every tool call, so the MCP reports an agent makes in that session bind to it — recorded work stops being reported-but-unjoinable. No live Kimi Code session has been observed recording over MCP with an inherited session id yet, and the hook bridge, while it has since fired on live desktop sessions (2026-09-24: 64 PreToolUse payloads across five sessions), has not yet been re-verified to join a real section to its session — the registration write and the hook bridge are covered by tests, and that live join is the one smoke still missing — so global onboarding reports it as experimental, like dsh. There is no Evidence v2 manifest adapter; the import lane is verified by a dated real-capture fixture plus a same-day single-machine live smoke (2026-09-23), so multi-version and long-run stability remain unclaimed and a non-zero cache-write path has still never been observed.
