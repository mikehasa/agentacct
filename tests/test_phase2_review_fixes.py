...
def _record_section(store_root, *, section_id, title, session, client="codex", transcript=None, status="completed"):
    metadata = {
        ...
        "progress": "Addressed the phase-2 review note for this step. Done." if status in {"completed", "handed_off"} else None,
    ...
    SentinelService(store_root).record_event(
        {
            ...
            "metadata": {
                ...
                "progress": "Addressed the phase-2 review note for this step. Done.",
            },
        }
    )
    SentinelService(store_root).record_event(
