...
Expected import, which is the sum of the two verbatim rows: model `DeepSeek/deepseek-flash`, input 596, output 190, cache read 44,800, cache write 0, two usage rows / two turns, cost unknown (Kimi Code persists no cost figure). The named evidence test is `tests/test_client_usage.py::test_discover_kimi_code_usage_reads_real_captured_wire_fixture`.
...
- **Not billing.** Tokens are client-reported and cost remains `unknown`; only a pricing-table estimate is possible.
