...
Fixtures prove the payload mapping, the config merge, the context shape (including the ancestry chain), and the selection rules; they do not prove that a real Kimi Code session fires the hook or that a real section inherits the id. No live hook firing has been observed yet, so `mechanical_capture` stays `experimental`.
