...
Generated from the capability manifest; last reviewed 2026-09-23. Integration breadth is not uniform verification: a client can have a proven usage lane and an experimental capture lane at the same time. Each lane is rated on its own, and the states are never flattened into one "supported" badge.
