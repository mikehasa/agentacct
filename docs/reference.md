...
## How the join keys arrive per client
...
- **Kimi Code** — the installed observe-only hook bridge captures Kimi Code's authoritative `session_id` from the hook payload and writes it to a hook-context file; the MCP side selects that context by the digest of its own working directory (pid lineage as the fallback), so reports made in the session bind to it. The bridge fires on live desktop sessions (measured 2026-09-24: 64 PreToolUse payloads across five sessions); a live MCP report inheriting a session id is still pending re-verification, and a session id is never guessed.
...
Use this when client auto-detection is unavailable or you need to configure each integration separately:
...
agentacct state is project-local under `.agent-sentinel/` (gitignored; the store directory keeps the pre-rename name `.agent-sentinel/` for compatibility with existing stores). Observe-only setup does not require provider API keys and does not connect to billing APIs. Open a **NEW** agent session after setup: MCP servers and hooks bind when the client session starts.
