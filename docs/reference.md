...
- **Kimi Code** — the installed observe-only hook bridge captures Kimi Code's authoritative `session_id` from the hook payload and writes it to a hook-context file; the MCP side selects that context by the client process's pid lineage, so reports made in the session bind to it. The bridge is fixture-verified only: no real Kimi Code session has fired it yet, and a session id is never guessed.
