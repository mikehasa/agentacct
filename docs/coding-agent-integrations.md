...
| Kimi Code | Global onboarding / `setup mcp --write` writes the user-level `mcp.json` with the machine-wide store and an absolute command (project config is not written) | `session_index.jsonl` plus per-agent `wire.jsonl` `usage.record` importer (per-request token deltas; no client cost figure, so cost comes only from the local pricing table) | Global onboarding / `hooks kimi-code install` merges an observe-only `[[hooks]]` bridge into the user's `config.toml`; the hook reports the authoritative session id (fixture-verified, no real session has fired it yet) | Usage import; MCP work context from the merged registration (recording not yet observed live, so onboarding reports it as experimental); hook ticks and hook-context files await a live session |
...
- **Installed client-specific v1 bridges.** Global onboarding installs Codex PreToolUse/SessionEnd hooks, Hermes shell hooks, an OpenCode plugin, and a Kimi Code observe-only `[[hooks]]` bridge merged into the user's `~/.kimi-code/config.toml`. They spool allowlisted activity/lifecycle facts and recognized check exit codes into the existing product/import paths; they never grant control or report token/cost truth. Codex needs one-time hook trust, Hermes needs one-time hook consent plus a gateway restart, and every client needs a new session. Codex/OpenCode transcript import may supersede overlapping hook activity rather than double-count it.
...
Kimi Code MCP registration is written by agentacct: Kimi Code declares servers as `mcpServers` entries in `~/.kimi-code/mcp.json` (`$KIMI_CODE_HOME/mcp.json`; project level `.kimi-code/mcp.json`) — never in `config.toml`, which carries provider credentials. `agentacct setup mcp --agent kimi-code --write` merges the agentacct entry into that **user-level** file (the project-level `.kimi-code/mcp.json` is not written) **with the machine-wide store and an absolute command path**: the file is loaded by every Kimi Code session on the machine, whatever directory it starts in, and a GUI-launched session does not inherit the shell PATH — so neither the invoking project's store nor a bare `agentacct` command belongs in it. That resolution is the same one `agentacct onboard --scope global --agent kimi-code` and `agentacct setup preview --agent kimi-code --user` use, so the preview and the write agree; an explicit absolute `--store-dir` is honored but warned about when it is not one of agentacct's machine-wide stores, and a relative one is refused (`--relative-store-path` too). The write preserves every other `mcpServers` entry and every other key, and collapses a standard pre-rename `agent-sentinel`/`agent-chronicle` entry into `agentacct` with its env carried over — a custom `agent-sentinel` server is left alone. `agentacct onboard --scope global --agent kimi-code` does the same registration and writes the standing record-your-work directive to `$KIMI_CODE_HOME/AGENTS.md` (`agentacct setup instructions --agent kimi-code`); without `--write`, `agentacct setup mcp --agent kimi-code` previews the exact entry, then start a new session because a running session never registers a newly added server. There is no generic Evidence v2 manifest adapter, and no live Kimi Code session has been observed recording over MCP yet — the registration write is covered by tests, the recording is not, so global onboarding reports kimi-code as experimental. The import lanes carry dated evidence from 2026-09-23 — a committed real-capture fixture (the two `usage.record` lines of a controlled run kept byte-verbatim, container metadata sanitized, expected import 596 input / 190 output / 44,800 cache read / 0 cache write on `DeepSeek/deepseek-flash`) plus a same-day single-machine live smoke in which the importer and an independent sum of the wire files agreed exactly on all six session×model lanes — so those lanes are `verified_partial` rather than synthetic-only. Cache write stays experimental: every captured and live lane reported 0, so a non-zero path has never been observed. Multi-version and long-run stability are not claimed.

Global onboarding also installs an observe-only hook bridge; `agentacct hooks kimi-code install` installs it on its own. Kimi Code runs its own `[[hooks]]` entries from `~/.kimi-code/config.toml`, so installation merges a PreToolUse command into that **user-level** file; a `[[hooks]]` entry accepts only four fields — `event`, `matcher`, `command`, and `timeout` — so nothing else is written there. The hook receives Kimi Code's authoritative `session_id` on stdin (never guessed) and writes a tool-activity tick plus a hook-context file on every tool call; the MCP side selects that context with bounded discriminators — a single fresh candidate, then a strict env match, then a unique working-directory **digest** of the session's `cwd` (the hook payload's cwd is stored only as a truncated SHA-256, never as a path), then unique pid lineage — so the reports an agent makes in that session bind to the authoritative session id the hook captured instead of arriving without a session. The digest is what carries the Kimi Code **desktop** app, which runs every session inside one main process: there the hook processes and the MCP server share that main pid, pid lineage ties for every candidate, and the session's own project directory is the only thing left that separates them. It never guesses: the digest decides only when every fresh candidate carries one and exactly one matches, and two sessions opened on the same directory refuse id inheritance outright. The bridge is opt-in because it edits the user's config file, and it is fixture-verified only: no real Kimi Code session has fired it yet, so the mechanical lane stays `experimental` until a live smoke exists.
