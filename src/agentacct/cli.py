...
evidence_rebuild_app = typer.Typer(
    help="Owner-gated stopped-writer snapshot, rebuild, activation, and rollback operations."
)
...
evidence_app.add_typer(evidence_rebuild_app, name="rebuild")
...
def _evidence_rebuild_receipt_json(
    path: Path,
    *,
    expected_name: str | None = None,
) -> tuple[Path, dict[str, Any]]:
    """Read one owner-private rebuild receipt without following symlinks."""

    requested = path.expanduser()
    if not requested.is_absolute():
        raise ValueError("rebuild receipt paths must be absolute")
    try:
        resolved = requested.resolve(strict=True)
        observed = requested.lstat()
        parent = requested.parent.lstat()
    except (OSError, RuntimeError) as exc:
        raise ValueError("rebuild receipt does not exist or is unsafe") from exc
    if resolved != requested or stat.S_ISLNK(observed.st_mode):
        raise ValueError("rebuild receipt may not contain symlink components")
    if expected_name is not None and requested.name != expected_name:
        raise ValueError(f"rebuild receipt must be named {expected_name}")
    if (
        ...
        raise ValueError("rebuild receipt must be one owner-only unique regular file")
    if (
        not stat.S_ISDIR(parent.st_mode)
        or parent.st_uid != os.geteuid()
        or stat.S_IMODE(parent.st_mode) & 0o077
    ):
        raise ValueError("rebuild receipt parent must be owner-only")
    if observed.st_size > _EVIDENCE_REBUILD_RECEIPT_MAX_BYTES:
        raise ValueError("rebuild receipt is unexpectedly large")
    flags = os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0)
    descriptor = os.open(requested, flags)
    try:
        opened = os.fstat(descriptor)
        if (opened.st_dev, opened.st_ino) != (observed.st_dev, observed.st_ino):
            raise ValueError("rebuild receipt changed before it was read")
        chunks: list[bytes] = []
        remaining = _EVIDENCE_REBUILD_RECEIPT_MAX_BYTES + 1
        while remaining:
        ...
        if (
            ...
            raise ValueError("rebuild receipt changed while it was read")
    finally:
        os.close(descriptor)
    ...
    def reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in result:
                raise ValueError("rebuild receipt contains duplicate JSON keys")
            result[key] = value
        return result
    ...
    try:
        ...
        raise ValueError("rebuild receipt is not strict UTF-8 JSON") from exc
    if not isinstance(value, dict):
        raise ValueError("rebuild receipt must be a JSON object")
    return resolved, value


def _evidence_rebuild_cli_refusal(
    action: str,
    error: BaseException,
    *,
    json_output: bool,
    ...
    if json_output:
        ...
        print(
            f"Evidence rebuild {action} refused: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
    raise typer.Exit(2)


@evidence_rebuild_app.command("snapshot")
def evidence_rebuild_snapshot(
    output_parent: Annotated[
        Path,
        typer.Option(
            "--output-parent",
            help="Existing owner-only directory outside every live agentacct state tree.",
        ),
    ],
    deployed_commit: Annotated[
        str,
        typer.Option(
            "--deployed-commit",
            help="Exact full lowercase 40-hex commit deployed for the stopped store.",
        ),
    ],
    store_dir: Annotated[Optional[Path], typer.Option(help=_STORE_DIR_HELP)] = None,
    confirm_writers_stopped: Annotated[
        ...
        typer.Option(
            ...
            help="Acknowledge that dashboard, watcher, MCP, hooks, and all other store writers are stopped.",
        ),
    ] = False,
    json_output: Annotated[bool, typer.Option("--json", help="Emit JSON evidence.")] = False,
) -> None:
    """Create and immediately verify one immutable private full-store snapshot."""

    from .evidence_rebuild_snapshot import create_evidence_snapshot

    resolved_store = _resolve_cli_store_dir(store_dir).path.resolve()
    snapshot_root = output_parent.expanduser() / (
        "evidence-snapshot-"
    ...
    try:
        result = create_evidence_snapshot(
            resolved_store,
            snapshot_root,
            confirm_writers_stopped=confirm_writers_stopped,
            deployed_commit=deployed_commit,
        )
    except Exception as exc:  # noqa: BLE001 - bounded owner-facing refusal.
        _evidence_rebuild_cli_refusal("snapshot", exc, json_output=json_output)
    payload = {"status": "verified_snapshot", **result.to_dict()}
    if json_output:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return
    print(f"Evidence snapshot verified: {result.snapshot_root}")
    print(f"Manifest SHA-256: {result.verification.manifest_sha256}")
    print(f"Tree digest: {result.verification.tree_digest}")
    print(f"Bytes: {result.verification.total_bytes}")
    print(f"Receipt: {result.receipt_path}")


@evidence_rebuild_app.command("verify-snapshot")
def evidence_rebuild_verify_snapshot(
    snapshot_receipt: Annotated[
        Path,
        typer.Option(
            "--snapshot-receipt",
            help="Absolute path to snapshot-receipt.json in a sealed snapshot.",
        ),
    ],
    restore_parent: Annotated[
        Path,
        typer.Option(
            "--restore-parent",
            help="Existing owner-only directory for a new byte-for-byte restore rehearsal.",
        ),
    ],
    json_output: Annotated[bool, typer.Option("--json", help="Emit JSON evidence.")] = False,
) -> None:
    """Verify a sealed snapshot and prove restoration into a separate private root."""
    ...
    try:
        receipt_path, _receipt = _evidence_rebuild_receipt_json(
            snapshot_receipt,
            expected_name=RECEIPT_FILENAME,
        )
        snapshot_root = receipt_path.parent
        verification = verify_evidence_snapshot(snapshot_root)
        restore_root = restore_parent.expanduser() / (
            "evidence-restore-rehearsal-"
            + datetime.now().strftime("%Y%m%d-%H%M%S")
            + "-"
            + uuid.uuid4().hex
        )
        rehearsal = rehearse_evidence_snapshot_restore(snapshot_root, restore_root)
    except Exception as exc:  # noqa: BLE001 - bounded owner-facing refusal.
        _evidence_rebuild_cli_refusal("snapshot verification", exc, json_output=json_output)
    payload = {
        "status": "restore_rehearsal_verified",
        "snapshot_verification": verification.to_dict(),
        "restore_rehearsal": rehearsal.to_dict(),
    }
    if json_output:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return
    print(f"Evidence snapshot verified: {verification.snapshot_root}")
    print(f"Restore rehearsal verified: {rehearsal.restore_root}")
    print(f"Manifest SHA-256: {verification.manifest_sha256}")
    print(f"Tree digest: {verification.tree_digest}")
    print(f"Bytes: {verification.total_bytes}")
    print(f"Rehearsal receipt: {rehearsal.receipt_path}")


@evidence_rebuild_app.command("build-candidate")
def evidence_rebuild_build_candidate(
    snapshot_receipt: Annotated[
        Path,
        typer.Option("--snapshot-receipt", help="Absolute sealed snapshot receipt path."),
    ],
    candidate_parent: Annotated[
        Path,
        typer.Option("--candidate-parent", help="Existing owner-only candidate parent."),
    ],
    codex_home: Annotated[Path, typer.Option("--codex-home", help="Explicit retained Codex home.")],
    claude_home: Annotated[
        Path,
        typer.Option("--claude-home", help="Explicit retained Claude Code home."),
    ],
    hermes_home: Annotated[Path, typer.Option("--hermes-home", help="Explicit retained Hermes home.")],
    all_retained_sessions: Annotated[
        bool,
        typer.Option(
            "--all-retained-sessions",
            help="Require the rebuild-only full-history importer rather than the normal bounded window.",
        ),
    ] = False,
    require_source_complete: Annotated[
        Optional[list[str]],
        typer.Option(
            "--require-source-complete",
            help="Client whose raw scan must be complete; repeat for codex, claude-code, and hermes.",
        ),
    ] = None,
    preserve_old_generic: Annotated[
        bool,
        typer.Option(
            "--preserve-old-generic",
            help="Conserve non-refreshable old Evidence and quarantine dangling claimed links.",
        ),
    ] = False,
    confirm_writers_stopped: Annotated[
        bool,
        typer.Option(
            "--confirm-writers-stopped",
            help="Acknowledge that agentacct and the selected raw clients are paused.",
        ),
    ] = False,
    confirm_owner_approved_rebuild: Annotated[
        bool,
        typer.Option(
            "--confirm-owner-approved-rebuild",
            help="Acknowledge the owner explicitly approved isolated candidate reconstruction.",
        ),
    ] = False,
    json_output: Annotated[bool, typer.Option("--json", help="Emit JSON evidence.")] = False,
) -> None:
    """Build a full-history candidate only after the explicit owner approval gate."""

    from .evidence_rebuild_candidate import (
        DEFAULT_REBUILD_CLIENTS,
        build_evidence_rebuild_candidate,
    )
    from .evidence_rebuild_snapshot import MANIFEST_FILENAME, RECEIPT_FILENAME

    required_clients = DEFAULT_REBUILD_CLIENTS
    try:
        if not confirm_owner_approved_rebuild:
            raise ValueError("--confirm-owner-approved-rebuild is required")
        if not confirm_writers_stopped:
            raise ValueError("--confirm-writers-stopped is required")
        if not all_retained_sessions:
            raise ValueError("--all-retained-sessions is required")
        if not preserve_old_generic:
            raise ValueError("--preserve-old-generic is required")
        required = tuple(require_source_complete or ())
        if len(required) != len(set(required)) or set(required) != set(required_clients):
            raise ValueError(
                "--require-source-complete must be repeated exactly once for codex, claude-code, and hermes"
            )
        receipt_path, _receipt = _evidence_rebuild_receipt_json(
            snapshot_receipt,
            expected_name=RECEIPT_FILENAME,
        )
        result = build_evidence_rebuild_candidate(
            receipt_path.parent,
            receipt_path.parent / MANIFEST_FILENAME,
            candidate_parent,
            ...
            # Stage banners go to stderr so --json stdout stays parseable; a
            # full-history build runs for many minutes and must never look
            # like a hang.
            progress=lambda message: print(
                f"[build-candidate] {message}", file=sys.stderr, flush=True
            ),
        )
    except Exception as exc:  # noqa: BLE001 - bounded owner-facing refusal.
        _evidence_rebuild_cli_refusal("candidate build", exc, json_output=json_output)
    if json_output:
        print(json.dumps(result, indent=2, sort_keys=True))
        return
    print(f"Evidence candidate status: {result.get('status')}")
    print(f"Candidate root: {result.get('candidate_root')}")
    print(f"Receipt: {result.get('receipt')}")
    print(f"Activation ready: {str(result.get('activation_ready') is True).lower()}")


@evidence_rebuild_app.command("verify-candidate")
def evidence_rebuild_verify_candidate(
    candidate_receipt: Annotated[
        Path,
        typer.Option("--candidate-receipt", help="Absolute candidate rebuild receipt path."),
    ],
    snapshot_receipt: Annotated[
        Path,
        typer.Option("--snapshot-receipt", help="Absolute sealed snapshot receipt path."),
    ...
    scratch_parent: Annotated[
        Path,
        typer.Option("--scratch-parent", help="Existing owner-only verification scratch parent."),
    ],
    json_output: Annotated[bool, typer.Option("--json", help="Emit JSON evidence.")] = False,
) -> None:
    """Rebuild a projection from candidate spools and verify logical parity/idempotence."""
    ...
    try:
        candidate_path, _candidate = _evidence_rebuild_receipt_json(
            candidate_receipt,
            expected_name=REBUILD_RECEIPT_FILENAME,
        )
        snapshot_path, _snapshot = _evidence_rebuild_receipt_json(
            snapshot_receipt,
            expected_name=RECEIPT_FILENAME,
        )
        result = verify_evidence_rebuild_candidate(
            candidate_path.parent,
            scratch_parent=scratch_parent,
            snapshot_root=snapshot_path.parent,
            snapshot_manifest=snapshot_path.parent / MANIFEST_FILENAME,
            required_clients=DEFAULT_REBUILD_CLIENTS,
        )
    except Exception as exc:  # noqa: BLE001 - bounded owner-facing refusal.
        _evidence_rebuild_cli_refusal("candidate verification", exc, json_output=json_output)
    if json_output:
        print(json.dumps(result, indent=2, sort_keys=True))
        return
    print(f"Evidence candidate verified: {result.get('candidate_root')}")
    print(f"Logical digest: {result.get('logical_digest')}")
    print(f"Activation ready: {str(result.get('activation_ready') is True).lower()}")


@evidence_rebuild_app.command("activate")
def evidence_rebuild_activate(
    candidate_receipt: Annotated[
        Path,
        typer.Option("--candidate-receipt", help="Absolute verified candidate receipt path."),
    ],
    snapshot_receipt: Annotated[
        Path,
        typer.Option("--snapshot-receipt", help="Absolute sealed pre-rebuild snapshot receipt path."),
    ],
    archive_parent: Annotated[
        Path,
        typer.Option("--archive-parent", help="Existing owner-only same-device archive parent."),
    ],
    receipt: Annotated[
        Path,
        typer.Option("--receipt", help="New absolute path for the durable activation receipt."),
    ],
    store_dir: Annotated[Optional[Path], typer.Option(help=_STORE_DIR_HELP)] = None,
    confirm_writers_stopped: Annotated[
        bool,
        typer.Option(
            "--confirm-writers-stopped",
            help="Acknowledge every agentacct and raw-client writer is stopped.",
        ),
    ] = False,
    confirm_owner_approved_activation: Annotated[
        bool,
        typer.Option(
            "--confirm-owner-approved-activation",
            help="Acknowledge the owner explicitly approved live Evidence replacement.",
        ),
    ] = False,
    json_output: Annotated[bool, typer.Option("--json", help="Emit JSON evidence.")] = False,
) -> None:
    """Atomically exchange the receipt-bound candidate and retain the old Evidence tree."""

    from .evidence_rebuild_activation import (
    ...
    from .evidence_rebuild_snapshot import MANIFEST_FILENAME, RECEIPT_FILENAME, verify_evidence_snapshot

    try:
        if not confirm_owner_approved_activation:
            raise ValueError("--confirm-owner-approved-activation is required")
        if not confirm_writers_stopped:
            raise ValueError("--confirm-writers-stopped is required")
        candidate_path, candidate_payload = _evidence_rebuild_receipt_json(
            candidate_receipt,
            expected_name=REBUILD_RECEIPT_FILENAME,
        )
        snapshot_path, snapshot_payload = _evidence_rebuild_receipt_json(
            snapshot_receipt,
            expected_name=RECEIPT_FILENAME,
        )
        if candidate_payload.get("activation_ready") is not True:
            raise ValueError("candidate receipt is not activation-ready")
        candidate_verification = verify_evidence_rebuild_candidate(
            candidate_path.parent,
            scratch_parent=candidate_path.parent.parent,
            snapshot_root=snapshot_path.parent,
            snapshot_manifest=snapshot_path.parent / MANIFEST_FILENAME,
            required_clients=DEFAULT_REBUILD_CLIENTS,
        )
        if candidate_verification.get("activation_ready") is not True:
            raise ValueError("candidate verification is not activation-ready")
        snapshot_verification = verify_evidence_snapshot(snapshot_path.parent)
        candidate_tree = (candidate_payload.get("candidate") or {}).get("evidence_tree")
        if not isinstance(candidate_tree, Mapping):
            raise ValueError("candidate receipt is missing its Evidence tree binding")
        candidate_evidence_path = (candidate_payload.get("candidate") or {}).get("evidence_path")
        if not isinstance(candidate_evidence_path, str):
            raise ValueError("candidate receipt is missing its Evidence path")
        candidate_evidence = Path(candidate_evidence_path)
        expected_candidate = candidate_path.parent / "evidence-v2"
        if candidate_evidence != expected_candidate:
            raise ValueError("candidate receipt Evidence path does not match its receipt root")
        live_tree = snapshot_payload.get("live_evidence_tree")
        if not isinstance(live_tree, Mapping):
            raise ValueError("snapshot receipt is missing its live Evidence tree binding")
        live = _resolve_cli_store_dir(store_dir).path.resolve() / "evidence-v2"
        archive = archive_parent.expanduser() / (
            "evidence-v2.archive-"
            + datetime.now().strftime("%Y%m%d-%H%M%S")
            + "-"
            + uuid.uuid4().hex
        )
        intent_path = receipt.expanduser().with_name(
            receipt.expanduser().name + ".intent.json"
        )
        intent = prepare_activation_intent(
            ...
            expected_live_tree_sha256=str(live_tree.get("tree_sha256") or ""),
            expected_live_total_bytes=live_tree.get("total_bytes"),
            expected_candidate_tree_sha256=str(candidate_tree.get("tree_sha256") or ""),
            expected_candidate_total_bytes=candidate_tree.get("total_bytes"),
            candidate_receipt_sha256=str(candidate_payload.get("receipt_sha256") or ""),
            snapshot_manifest_sha256=snapshot_verification.manifest_sha256,
            require_receipt_bindings=True,
        )
        result = activate_evidence_rebuild(
            intent_path,
            final_receipt_path=receipt,
            confirm_writers_stopped=True,
        )
    except EvidenceActivationPostSwapError as exc:
        payload = {
            "status": "activation_post_swap_failed",
            "state": exc.state,
        ...
        if json_output:
            ...
            print("CRITICAL: Evidence activation crossed or may have crossed the atomic swap.", file=sys.stderr)
            print(json.dumps(payload, indent=2, sort_keys=True), file=sys.stderr)
        raise typer.Exit(2)
    except Exception as exc:  # noqa: BLE001 - fail closed before replacement.
        _evidence_rebuild_cli_refusal("activation", exc, json_output=json_output)
    payload = {
        "status": "activated",
        "receipt_path": str(receipt.expanduser()),
        "activation": result.to_dict(),
    }
    if json_output:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return
    print(f"Evidence candidate activated: {result.installed_live.path}")
    print(f"Previous Evidence retained: {result.archived_previous_live.path}")
    print(f"Activation receipt: {receipt.expanduser()}")


@evidence_rebuild_app.command("inspect-activation")
def evidence_rebuild_inspect_activation(
    activation_intent: Annotated[
        Path,
        typer.Option(
            "--activation-intent",
            help="Absolute durable activation intent left by prepare/activate.",
        ),
    ],
    json_output: Annotated[bool, typer.Option("--json", help="Emit JSON evidence.")] = False,
) -> None:
    """Classify an interrupted activation without changing any path."""

    from .evidence_rebuild_activation import inspect_activation_state

    try:
        state = inspect_activation_state(activation_intent)
    except Exception as exc:  # noqa: BLE001 - bounded owner-facing refusal.
        _evidence_rebuild_cli_refusal("activation crash inspection", exc, json_output=json_output)
    payload = {
        "status": "inspected",
        "operation": "activation",
        "state": state,
        "recoverable": state != "ambiguous",
        "mutation_required": state in {"pre_swap", "swapped_pending_archive"},
    }
    if json_output:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return
    print(f"Activation crash state: {state}")
    print(f"Recoverable: {str(payload['recoverable']).lower()}")


@evidence_rebuild_app.command("recover-activation")
def evidence_rebuild_recover_activation(
    activation_intent: Annotated[
        Path,
        typer.Option(
            "--activation-intent",
            help="Absolute durable activation intent to resume or finalize.",
        ),
    ],
    receipt: Annotated[
        Path,
        typer.Option("--receipt", help="New absolute path for the final activation receipt."),
    ],
    confirm_writers_stopped: Annotated[
        bool,
        typer.Option(
    ...
    confirm_owner_approved_activation: Annotated[
        ...
        typer.Option(
            "--confirm-owner-approved-activation",
            help="Acknowledge owner approval to resume/finalize this activation.",
        ),
    ] = False,
    json_output: Annotated[bool, typer.Option("--json", help="Emit JSON evidence.")] = False,
) -> None:
    """Resume a classifiable activation crash or publish its missing receipt."""
    ...
    try:
        ...
        if not confirm_writers_stopped:
            raise ValueError("--confirm-writers-stopped is required")
        result = recover_activation_from_intent(
            activation_intent,
            final_receipt_path=receipt,
            confirm_writers_stopped=True,
        )
        ...
        payload = {
            "status": "activation_recovery_post_swap_failed",
            "state": exc.state,
            "error_type": type(exc).__name__,
            "error": str(exc)[:1000],
            "intent": exc.intent.to_dict(),
        ...
        if json_output:
            ...
            print("CRITICAL: Evidence activation recovery remains post-swap.", file=sys.stderr)
            print(json.dumps(payload, indent=2, sort_keys=True), file=sys.stderr)
        raise typer.Exit(2)
    except Exception as exc:  # noqa: BLE001 - fail closed before any unbound action.
        _evidence_rebuild_cli_refusal("activation crash recovery", exc, json_output=json_output)
    payload = {
        "status": "activation_recovered",
        "receipt_path": str(receipt.expanduser()),
        "activation": result.to_dict(),
    }
    if json_output:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return
    print(f"Evidence activation recovered: {result.installed_live.path}")
    print(f"Previous Evidence retained: {result.archived_previous_live.path}")
    print(f"Activation receipt: {receipt.expanduser()}")


@evidence_rebuild_app.command("verify-activation")
def evidence_rebuild_verify_activation(
    activation_receipt: Annotated[
        Path,
        typer.Option("--activation-receipt", help="Absolute durable activation receipt path."),
    ],
    confirm_writers_stopped: Annotated[
        bool,
        typer.Option("--confirm-writers-stopped", help="Acknowledge writers remain stopped for exact verification."),
    ] = False,
    json_output: Annotated[bool, typer.Option("--json", help="Emit JSON evidence.")] = False,
) -> None:
    """Verify the installed and archived Evidence directory identities and digests."""

    from .evidence_rebuild_activation import verify_evidence_activation
    ...
    try:
        ...
        _evidence_rebuild_cli_refusal("activation verification", exc, json_output=json_output)
    payload = {
    ...
    if json_output:
        ...
        print("Evidence activation verified.")
    else:
        print("Evidence activation verification blocked.", file=sys.stderr)
...
@evidence_rebuild_app.command("rollback")
def evidence_rebuild_rollback(
    activation_receipt: Annotated[
        Path,
        typer.Option("--activation-receipt", help="Absolute durable activation receipt path."),
    ],
    failed_archive_parent: Annotated[
        Path,
        typer.Option(
            "--failed-archive-parent",
            help="Owner-only same-device parent retaining the failed post-activation tree.",
        ),
    ],
    receipt: Annotated[
        Path,
        typer.Option("--receipt", help="New absolute path for the durable rollback receipt."),
    ],
    confirm_writers_stopped: Annotated[
        bool,
        typer.Option("--confirm-writers-stopped", help="Acknowledge every writer is stopped again."),
    ] = False,
    confirm_owner_approved_rollback: Annotated[
        bool,
        typer.Option(
            "--confirm-owner-approved-rollback",
            help="Acknowledge the owner explicitly approved the rollback exchange.",
        ),
    ] = False,
    json_output: Annotated[bool, typer.Option("--json", help="Emit JSON evidence.")] = False,
) -> None:
    """Atomically restore the prior Evidence tree while retaining the failed generation."""
    ...
    try:
        if not confirm_owner_approved_rollback:
            raise ValueError("--confirm-owner-approved-rollback is required")
        if not confirm_writers_stopped:
            raise ValueError("--confirm-writers-stopped is required")
        suffix = datetime.now().strftime("%Y%m%d-%H%M%S") + "-" + uuid.uuid4().hex
        failed_archive = failed_archive_parent.expanduser() / f"evidence-v2.failed-{suffix}"
        intent_path = receipt.expanduser().with_name(receipt.expanduser().name + ".intent.json")
        result = rollback_evidence_activation(
            activation_receipt,
    ...
    except EvidenceRollbackPostSwapError as exc:
        payload = {
            "status": "rollback_post_swap_failed",
            "state": exc.state,
            "error_type": type(exc).__name__,
            "error": str(exc)[:1000],
            "intent": exc.intent.to_dict(),
        ...
        if json_output:
            ...
            print("CRITICAL: Evidence rollback crossed or may have crossed the atomic swap.", file=sys.stderr)
            print(json.dumps(payload, indent=2, sort_keys=True), file=sys.stderr)
        raise typer.Exit(2)
    except Exception as exc:  # noqa: BLE001 - fail closed before replacement.
        _evidence_rebuild_cli_refusal("rollback", exc, json_output=json_output)
    payload = {"status": "rolled_back", "rollback": result.to_dict()}
    if json_output:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return
    print(f"Prior Evidence restored: {result.restored_live.path}")
    print(f"Failed generation retained: {result.failed_live_archive.path}")
    print(f"Rollback receipt: {receipt.expanduser()}")


@evidence_rebuild_app.command("inspect-rollback")
def evidence_rebuild_inspect_rollback(
    rollback_intent: Annotated[
        Path,
        typer.Option(
            "--rollback-intent",
            help="Absolute durable rollback intent left by an interrupted rollback.",
        ),
    ],
    json_output: Annotated[bool, typer.Option("--json", help="Emit JSON evidence.")] = False,
) -> None:
    """Classify an interrupted rollback without changing any path."""

    from .evidence_rebuild_activation import inspect_rollback_state

    try:
        state = inspect_rollback_state(rollback_intent)
    except Exception as exc:  # noqa: BLE001 - bounded owner-facing refusal.
        _evidence_rebuild_cli_refusal("rollback crash inspection", exc, json_output=json_output)
    payload = {
        "status": "inspected",
        "operation": "rollback",
        "state": state,
        "recoverable": state != "ambiguous",
        "mutation_required": state in {"pre_swap", "swapped_pending_failed_archive"},
    }
    if json_output:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return
    print(f"Rollback crash state: {state}")
    print(f"Recoverable: {str(payload['recoverable']).lower()}")


@evidence_rebuild_app.command("recover-rollback")
def evidence_rebuild_recover_rollback(
    rollback_intent: Annotated[
        Path,
        typer.Option(
            "--rollback-intent",
            help="Absolute durable rollback intent to resume or finalize.",
        ),
    ],
    receipt: Annotated[
        Path,
        typer.Option("--receipt", help="New absolute path for the final rollback receipt."),
    ],
    confirm_writers_stopped: Annotated[
        bool,
        typer.Option(
    ...
    confirm_owner_approved_rollback: Annotated[
        ...
        typer.Option(
            "--confirm-owner-approved-rollback",
            help="Acknowledge owner approval to resume/finalize this rollback.",
        ),
    ] = False,
    json_output: Annotated[bool, typer.Option("--json", help="Emit JSON evidence.")] = False,
) -> None:
    """Resume a classifiable rollback crash or publish its missing receipt."""
    ...
    try:
        ...
        if not confirm_writers_stopped:
            raise ValueError("--confirm-writers-stopped is required")
        result = recover_rollback_from_intent(
            rollback_intent,
            final_receipt_path=receipt,
            confirm_writers_stopped=True,
        )
        ...
        payload = {
            "status": "rollback_recovery_post_swap_failed",
            "state": exc.state,
            "error_type": type(exc).__name__,
            "error": str(exc)[:1000],
            "intent": exc.intent.to_dict(),
        ...
        if json_output:
            ...
            print("CRITICAL: Evidence rollback recovery remains post-swap.", file=sys.stderr)
            print(json.dumps(payload, indent=2, sort_keys=True), file=sys.stderr)
        raise typer.Exit(2)
    except Exception as exc:  # noqa: BLE001 - fail closed before any unbound action.
        _evidence_rebuild_cli_refusal("rollback crash recovery", exc, json_output=json_output)
    payload = {"status": "rollback_recovered", "rollback": result.to_dict()}
    if json_output:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return
    print(f"Prior Evidence restored: {result.restored_live.path}")
