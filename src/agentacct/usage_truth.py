...
USAGE_TRUTH_TABLE: tuple[UsageTruthRow, ...] = (
    ...
    UsageTruthRow(
        integration="dsh (DeepSeek Harness) session-log usage import",
        tier="local_import",
        evidence_source="DeepSeek Harness Zstandard-compressed JSONL session logs under $DSH_HOME (default ~/.dsh)",
        update_timing="dsh assistant/message events carry data.usage token fields (inputTokens/outputTokens/cacheReadTokens/cacheWriteTokens/reasoningTokens); the log records no cost figure.",
        observable_fields=[
            "session id from the session header",
            "provider/model from the assistant message source",
        ...
        limitations=[
            "dsh does not persist a cost in its logs; only agentacct's local pricing-table estimate is available, never a provider invoice.",
            "Does not store prompts or transcript content.",
            "Only covers logs present in the configured dsh roots.",
            "Field paths were verified against dsh source and third-party parsers, not yet a live-client smoke on this machine.",
        ],
    ),
    UsageTruthRow(
